...
if [ `id -u` -ne 0 ] ; then
  echo "FATAL! I need to run as root."
  exit 1
fi
...
# Setup helm (and its plugins). Run this as "runner" which is the default user
# in a github actions container. This process builds up the user's $HOME/.helm
# directory, which we'll tar up later.
...
groupadd -g $GHA_GID $GHA_GROUP
useradd -g $GHA_GID -u $GHA_UID -d /home/$GHA_USER -m $GHA_USER
chown $GHA_USER /usr/local/bin

su - $GHA_USER bash -c "

  # Note that the tiller'less helm plugin needs the 'tiller' binary. This is
  # why we move the 'tiller' binary into that plugin's directory.

  HELM_TAR_SRC=https://get.helm.sh/helm-v2.16.5-linux-amd64.tar.gz
  HELM_TAR_DST=/tmp/helm.tar.gz
...
tar -cvpf $TOOLS_TAR /home/$GHA_USER/.helm /usr/local/bin/* || exit 1
